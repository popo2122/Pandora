# Pandora 사이트 리뉴얼    
> 사이트에 사용된 모든 사진은 상업적 목적이 아닌, 포트폴리오 제작용으로 사용되었습니다.
...
## *🌸 Design*
 - 브랜드가 가진 특유의 따뜻한 분위기와 브랜드를 대표하는 색인 '핑크'를 전체적인 포인트 색으로 지정하여 디자인
 - 화려하지 않으면서도, 하늘하늘한 느낌을 더하고 싶어 핑크색 찢어진듯한 천 느낌을 배경으로 배치
 - 전체적으로 심플하지만, 기존 사이트에서 잘 보이지 않았던 제품과 이미지를 첫 페이지에서 강조하기 위해   
   텍스트를 최대한 줄이고 제품/카달로그 이미지 위주로 배치
 - 로고를 제외한 페이지 내의 아이콘은 일러스트레이터로 직접 제작
...
- 제품 이미지 슬라이드와 모바일 버전의 메인 슬라이드는 Swiper 라이브러리를 활용
